 ...
 */
public class StateTag extends BodyTagSupport {
	...
	public int doEndTag() throws JspException {
		String state = getBodyContent().getString().trim();
		...
		if(storeCreditCardTag!=null) {
			storeCreditCardTag.setState(state);
		} else {
			...
			if(creditCardTag!=null) {
				...
				paymentTag.setCreditCardState(state);
			} else {
				...
				if(shippingAddressTag!=null) {
					...
					paymentTag.setShippingAddressState(state);
				} else {
					throw new JspException("state tag must be within a storeCreditCard tag, creditCard tag, or shippingAddress tag");
				}
			}
		}
