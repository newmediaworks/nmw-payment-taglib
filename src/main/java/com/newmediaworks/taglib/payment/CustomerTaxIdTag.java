...
public class CustomerTaxIdTag extends BodyTagSupport {
	...
	public int doEndTag() throws JspException {
		String customerTaxId = getBodyContent().getString().trim();
		...
		if(storeCreditCardTag!=null) {
			storeCreditCardTag.setCustomerTaxId(customerTaxId);
		} else {
			...
			if(creditCardTag!=null) {
				...
				paymentTag.setCreditCardCustomerTaxId(customerTaxId);
			} else {
				throw new JspException("customerTaxId tag must be within either a storeCreditCard tag or a creditCard tag");
			}
		}
