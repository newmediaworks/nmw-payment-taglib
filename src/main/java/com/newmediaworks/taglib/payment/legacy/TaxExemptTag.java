...
public class TaxExemptTag extends EncodingBufferedBodyTag {
  ...
  protected int doEndTag(BufferResult capturedBody, Writer out) throws JspException, IOException {
    ...
    if (value != null) {
      ...
      if ("true".equalsIgnoreCase(taxExemptString)) {
        taxExempt = true;
      } else if ("false".equalsIgnoreCase(taxExemptString)) {
        taxExempt = false;
      } else {
        throw new JspTagException("Invalid value for " + TAG_NAME + ", should be either true or false: " + taxExemptString);
      }
    }
